...
std::unique_ptr<sentencia_declaracion> stmt_declaracion(const token_anotado*& p, std::unique_ptr<expresion> tipo, bool permitir_init) {
   ...
   auto init = (permitir_init && p->tipo == ASIGNACION ? expr(++p) : nullptr);
   return std::make_unique<sentencia_declaracion>(std::move(tipo), *nombre, std::move(init));
}

std::unique_ptr<sentencia_declaracion> stmt_declaracion(const token_anotado*& p, bool permitir_init) {
   return stmt_sentencia(p, expr(p), permitir_init);
}
...
std::unique_ptr<sentencia> stmt(const token_anotado*& p) {
   if (p->tipo == IF) {
      auto ex = expr(++p);
      espera(p, LLAVE_IZQ);
      auto parte_si = lista_stmt(p), parte_no = std::vector<std::unique_ptr<sentencia>>( );
      espera(p, LLAVE_DER);
      if (p->tipo == ELSE) {
         if ((++p)->tipo == IF) {
      ...
      if (p->tipo == IDENTIFICADOR) {
         auto decl = stmt_declaracion(p, std::move(ex), true);
         espera(p, PUNTO_COMA);
         return decl;
      } else {
         espera(p, PUNTO_COMA);
